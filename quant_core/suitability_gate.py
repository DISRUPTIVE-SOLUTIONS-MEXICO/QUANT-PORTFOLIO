...
def evaluate_suitability_gate(config, portfolio: pd.DataFrame, performance_summary: pd.DataFrame, diagnostics: pd.DataFrame | None = None) -> dict:
    ...
    if pd.isna(vol) and portfolio is not None and not portfolio.empty and "realized_weight_ann_vol" in portfolio:
        vol = pd.to_numeric(portfolio["realized_weight_ann_vol"], errors="coerce").dropna().iloc[0] if pd.to_numeric(portfolio["realized_weight_ann_vol"], errors="coerce").notna().any() else np.nan
    if pd.isna(cvar) and portfolio is not None and not portfolio.empty and "hist_cvar_95_daily" in portfolio:
        cvar = pd.to_numeric(portfolio["hist_cvar_95_daily"], errors="coerce").dropna().iloc[0] if pd.to_numeric(portfolio["hist_cvar_95_daily"], errors="coerce").notna().any() else np.nan
    if pd.isna(max_dd) and portfolio is not None and not portfolio.empty and "realized_weight_max_drawdown" in portfolio:
        max_dd = pd.to_numeric(portfolio["realized_weight_max_drawdown"], errors="coerce").dropna().iloc[0] if pd.to_numeric(portfolio["realized_weight_max_drawdown"], errors="coerce").notna().any() else np.nan
    ...
    weights = pd.to_numeric(portfolio.get("Weight", pd.Series(dtype=float)), errors="coerce") if portfolio is not None and not portfolio.empty else pd.Series(dtype=float)
    breaches = []
    ...
    if pd.notna(limits["CVaR_Max_Daily"]) and pd.notna(cvar) and cvar > limits["CVaR_Max_Daily"]:
        breaches.append({"Constraint": "Daily CVaR", "Observed": cvar, "Limit": limits["CVaR_Max_Daily"], "Severity": "hard"})
    if pd.notna(limits["DD_Max"]) and pd.notna(max_dd) and abs(max_dd) > limits["DD_Max"]:
        breaches.append({"Constraint": "Max drawdown", "Observed": max_dd, "Limit": -abs(limits["DD_Max"]), "Severity": "hard"})
    if pd.notna(limits["Max_Weight"]) and not weights.empty and weights.max() > limits["Max_Weight"] + 1e-8:
        breaches.append({"Constraint": "Single-name weight", "Observed": float(weights.max()), "Limit": limits["Max_Weight"], "Severity": "hard"})
