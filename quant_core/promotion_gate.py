...
def evaluate_promotion_gate(
    ...
    dd_limit = abs(float(getattr(config, "investor_max_drawdown", np.nan))) if pd.notna(getattr(config, "investor_max_drawdown", np.nan)) else np.nan
    cvar_limit = abs(float(getattr(config, "investor_cvar_max_daily", np.nan))) if pd.notna(getattr(config, "investor_cvar_max_daily", np.nan)) else np.nan
    checks = [
    ...
    if pd.notna(dd_limit):
        checks.append(("Max drawdown", max_dd, dd_limit, bool(pd.notna(max_dd) and abs(max_dd) <= dd_limit), "lower_abs"))
    if pd.notna(cvar_limit):
        checks.append(("Daily CVaR 95%", cvar95, cvar_limit, bool(pd.notna(cvar95) and abs(cvar95) <= cvar_limit), "lower_abs"))
    rows = []
