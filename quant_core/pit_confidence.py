...
def add_pit_confidence(panel: pd.DataFrame, asof_date=None, ratio_cols: tuple[str, ...] = RATIO_COLUMNS) -> pd.DataFrame:
    ...
    has_accept = pd.to_datetime(out.get("SEC_Accepted_At"), errors="coerce").notna() if "SEC_Accepted_At" in out else pd.Series(False, index=out.index)
    ...
    base = 0.20 + 0.35 * has_sec.astype(float) + 0.20 * has_accept.astype(float) + 0.20 * coverage - 0.20 * stale_penalty
    ...
    for ratio in ratio_cols:
        if ratio in out:
            out[f"{ratio}_PIT_Confidence"] = np.where(pd.to_numeric(out[ratio], errors="coerce").notna(), out["PIT_Confidence"], 0.0)
            out[f"{ratio}_Missing_Reason"] = np.where(pd.to_numeric(out[ratio], errors="coerce").notna(), "", "missing_or_not_applicable")
